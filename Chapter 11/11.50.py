...
def distance(x1, y1, x2, y2):
    return ((x2 - x1) * (x2 - x1) + (y2-11) * (y2- y1)) ** 0.5
...
class NearestPointsGUI:
    def __init__(self):
        self.RADIUS = 2 # radius of the point
    ...
    def addThisPoint (self, x, y):
        ...
        self.canvas.create_oval(x - self.RADIUS, y - self.RADIUS,
            x + self.RADIUS+1, y + self.RADIUS+1,fill="red")
        ...
        if len(self.points) > 2:
            ...
            self.canvas.create_line(self.points[p1][0],
                ...
                self.points[p2][1], fill = "black", tags = "line")


    def isTooCloseToOtherPoints(self, x, y) :
        for i in range(len(self.points)): 
            if distance(x,y,self.points[i][0], self.points[i][1]) <= self.RADIUS + 2: 
                return True
